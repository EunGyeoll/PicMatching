  ...
  if (!photographer) {
    return (
      <main className="mx-auto flex min-h-dvh max-w-120 flex-col items-center justify-center gap-3 px-6 text-center">
        ...
        <Link href="/explore" className="text-base text-stone-900 underline">
          탐색으로 돌아가기
        </Link>
      </main>
    );
  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-120 flex-col pb-24">
      <div className="flex flex-col items-center gap-2 px-6 pt-8 pb-4 text-center">
        ...
        <h1 className="max-w-full wrap-break-word text-base font-bold text-stone-900">
          {photographer.displayName}
        </h1>
        {photographer.headline ? (
          <p className="max-w-60 wrap-break-word text-sm text-stone-500">{photographer.headline}</p>
        ) : null}
      ...
      <div className="flex border-y border-stone-200">
        <Link
          ...
          className={`flex-1 border-b-2 py-2.5 text-center text-xs font-bold ${
            activeTab === "portfolio"
            ...
            photographer.services.map((service) => (
              ...
              >
                ...
                <div className="flex min-w-0 flex-1 flex-col gap-1">
                  <Link
                    href={`/photographers/${id}/services/${service.id}`}
                    className="truncate text-sm font-bold text-stone-900"
                  >
                    {service.title}
                  </Link>
                  ...
                  <Link
                    ...
                    className="mt-1 self-start rounded-full border border-stone-800 px-2.5 py-2 text-xs font-semibold text-stone-800"
                  >
                    이 서비스로 예약
                  </Link>
                </div>
      ...
      {photographer.bio ? (
        <div className="flex flex-col gap-1.5 border-t border-stone-100 px-4 py-4">
          <h2 className="text-sm font-bold text-stone-400">소개</h2>
      ...
      {startingPrice !== null ? (
        <div className="fixed inset-x-0 bottom-0 z-10 mx-auto flex max-w-120 items-center justify-between border-t border-stone-200 bg-white px-4 py-3 pb-[max(12px,env(safe-area-inset-bottom))]">
          <div className="flex flex-col">
            <span className="text-xs text-stone-400">시작가</span>
...
function Stat({ value, label }: { value: number | string; label: string }) {
  return (
    <div className="flex flex-col items-center gap-0.5">
      <span className="text-base font-extrabold text-stone-900">{value}</span>
